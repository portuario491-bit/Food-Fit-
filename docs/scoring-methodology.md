...
## 9. Datos: universo real del IBEX35 (y modo de demostración)

Por defecto (`FINANCIAL_DATA_PROVIDER=ibex35`), la aplicación usa el universo real de las 35 empresas del IBEX35, con cifras recopiladas manualmente de fuentes públicas (Investing.com, MarketScreener, informes de las propias empresas...) a principios de septiembre de 2026. **No hay actualización automática ni en tiempo real todavía**: es una fotografía manual. Cuando una cifra no se pudo verificar con confianza, el campo queda en `null` ("sin dato" en la interfaz) en vez de estimarse — por eso varias empresas muestran sub-scores calculados con pocas métricas disponibles, señalado explícitamente en la interfaz. El detalle de fuente y fecha por empresa está en `/docs/ibex35-fuentes.md`.

También existe un modo de demostración (`FINANCIAL_DATA_PROVIDER=mock`) con un universo de empresas **completamente ficticias**, marcadas con `isMock: true`, diseñadas para representar arquetipos (una aristócrata del dividendo, una "trampa de yield" con payout insostenible, una tecnológica cara de alto crecimiento, un banco con métricas no comparables, etc.) y así poder validar que la metodología reacciona como se espera ante cada caso extremo, con datos completos en todas las métricas. Estos datos ficticios nunca deben interpretarse como información de mercado real.
