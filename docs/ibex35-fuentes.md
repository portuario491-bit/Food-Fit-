...
## Ampliación del universo — S&P 500 (cartera personal)

Investigación de 7 empresas estadounidenses (S&P 500) recién añadidas a `lib/data/providers/globalCompanies.ts` (fichero de ampliación del universo, distinto de `ibex35Companies.ts`), posiciones reales de la cartera personal del propietario del sitio: JNJ, GIS, T, GOOGL, KO, MSFT, WBD. Recopilación por búsqueda web (Claude WebSearch) el 8-sep-2026. Igual que en las rondas del IBEX35, el fetch directo de páginas (stockanalysis.com, sec.gov, wikipedia.org, etc.) devolvió `EGRESS_BLOCKED` en este entorno, tanto vía WebFetch como vía `curl` directo (confirmado con `curl -sS .../__agentproxy/status`, que muestra el dominio rechazado por política de la organización) — todos los datos proceden de resúmenes/snippets de WebSearch que citan esas fuentes, contrastados entre sí cuando ha sido posible. **NOTA: en una primera pasada de esta ronda el cupo de búsquedas web de la sesión se agotó tras completar 5 de las 7 empresas, dejando MSFT y WBD sin investigar; una ronda de seguimiento (misma fecha, 8-sep-2026) completó ambas fichas con cupo de búsquedas disponible — ver sus entradas más abajo.**

### Johnson & Johnson (JNJ)
- Añadido: precio ($275,23) y marketCap ($663.280M) de finance.yahoo.com; `dividendYield` (1,95%), `payoutRatioEarnings` (52%), `payoutRatioFCF` (56%, cálculo propio: dividendos pagados ~$12.400M / FCF TTM $22.240M), `consecutiveYearsPaying` (82, desde 1944), `consecutiveYearsIncreasing` (64, Dividend King — jnj.com press release abril 2026), `dividendCagr5y` (5,3%), `dividendCagr10y` (5,9%, aproximado), `dividendCoverage` (1,79), `epsGrowthCagr5y` (6,8%), `roic` (19,92%), `roe` (25,74%), `operatingMargin` (27,69%), `netMargin` (21,48%), `netDebtToEbitda` (0,71, cálculo propio: deuda neta $28.200M / EBITDA TTM ~$39.500M), `per` (31,97), `forwardPer` (22,98), `peg` (2,96), `evEbitda` (19,49), `priceToFcf` (32,0), `priceToSales` (5,8), `beta` (0,23, confirmado dos veces en Yahoo Finance 5Y monthly), `return6m` (38%), `return1y` (55%), `return3yCagr` (19,68%), `distanceFromHigh52w` (1%), `aboveSma200` (true). Fuentes: finance.yahoo.com, jnj.com, macrotrends.net, gurufocus.com, stockanalysis.com, tikr.com, 247wallst.com, fool.com. Consulta: 8-sep-2026.
- Se descartó un beta de -0,53 encontrado en una fuente por ser un outlier no reconciliable frente a las dos confirmaciones de Yahoo Finance (0,23).
- Dejado en null: `dividendCagr3y`, `revenueGrowthCagr3y/5y` (solo se encontró guía forward, no CAGR histórico limpio), `epsGrowthCagr3y`, `fcfGrowthCagr3y`, `operatingMarginTrend5y`, `earningsStability`, `interestCoverage` (datos contradictorios/no estándar), `perVsHistoricalAvg5y`, `volatility3y`, `maxDrawdown5y`.

### General Mills (GIS)
- AÑO FISCAL 2026 MUY DISTORSIONADO: pérdida neta GAAP de -$87,6M (año fiscal cerrado 31-may-2026) por $2.530M en deterioros no monetarios (venta de Brasil -$1.030M, fondo de comercio de mascotas Norteamérica -$1.500M) — generalmills.com press release 1-jul-2026.
- Añadido: precio ($38,29) y marketCap ($20.470M, consistente con 534,63M acciones), `dividendYield` (6,4%), `payoutRatioEarnings` (53%), `payoutRatioFCF` (57%), `consecutiveYearsPaying` (127, sin interrupción desde 1898, bien documentado), `dividendCagr3y` (5,6%), `dividendCagr5y` (4,5%), `dividendCoverage` (1,75), `operatingMargin` (15,3%, margen operativo AJUSTADO oficial FY2026 — el GAAP está distorsionado por los deterioros), `per` (10,8) y `forwardPer` (10,6) calculados sobre el EPS diluido AJUSTADO oficial de $3,55 (el GAAP es negativo), `evEbitda` (9,96), `priceToFcf` (8,95, cálculo propio), `priceToSales` (1,10), `beta` (-0,03, confirmado en Yahoo Finance, inusualmente bajo pero corroborado), `return1y` (-37,8%), `distanceFromHigh52w` (25,4%), `aboveSma200` (false, precio $38,29 bajo la SMA200 de $40,56). Fuentes: stockanalysis.com, generalmills.com press releases, gurufocus.com, finance.yahoo.com, tikr.com, wealthyhood.com. Consulta: 8-sep-2026.
- `roe`, `roic`, `netMargin`, `netDebtToEbitda`, `interestCoverage` dejados en NULL pese a encontrarse cifras (roe 23,6%, roic 9,3%): corresponden a caché desactualizado previo al informe de resultados de julio 2026 y no son coherentes con la pérdida GAAP oficial ya documentada.
- `consecutiveYearsIncreasing` dejado en NULL por contradicción real entre fuentes: 7, 19 y "25+" años consecutivos de subida citados sin poder reconciliarse (General Mills no es Dividend Aristocrat pese a su largo historial de pago ininterrumpido).
- Dejado en null también: `dividendCagr10y`, `revenueGrowthCagr3y/5y` y `epsGrowthCagr3y/5y` (no comparables con la caída de ingresos -5% de FY2026), `fcfGrowthCagr3y`, `operatingMarginTrend5y`, `earningsStability`, `peg` (fuente explícita: no disponible/no significativo), `perVsHistoricalAvg5y`, `volatility3y`, `maxDrawdown5y`, `return6m`, `return3yCagr`.

### AT&T (T)
- HISTORIA DE DIVIDENDO CON DISCONTINUIDAD IMPORTANTE (nota especial solicitada): AT&T recortó su dividendo anual casi a la mitad en 2022 (de $2,08 a $1,11/acción) al escindir WarnerMedia (fusionada con Discovery para formar la propia WBD de esta lista) — perdió su condición de Dividend Aristocrat tras 35+ años de mantener o subir el dividendo. En 2026 subió el dividendo trimestral por primera vez desde el recorte (de $0,2775 a $0,28/acción) — fuente: cnbc.com, about.att.com, fool.com.
- Añadido: precio ($25,68) y marketCap ($175.970M, 6-sep-2026), `dividendYield` (4,24%), `payoutRatioEarnings` (36%) y `payoutRatioFCF` (45%, cálculo propio a partir de EPS TTM ~$3,11 y FCF guiado ~$18.000M), `consecutiveYearsIncreasing` (1, primera subida en 4 años), `dividendCoverage` (2,2), `roic` (8,58%), `roe` (18,37%), `operatingMargin` (21,3%), `netMargin` (16,94%), `netDebtToEbitda` (2,63, anualizado Q1 2026), `per` (7,3), `forwardPer` (9,66), `evEbitda` (6,68), `priceToFcf` (10,1, cálculo propio), `priceToSales` (1,3), `beta` (0,39). Fuentes: stockanalysis.com, gurufocus.com, finance.yahoo.com, about.att.com, fool.com, cnbc.com, tipranks.com. Consulta: 8-sep-2026.
- `consecutiveYearsPaying` dejado en NULL: la historia corporativa de "AT&T" (fusión con SBC en 2005, la escisión de 2022) no permite fijar un número único fiable de años de pago ininterrumpido.
- CONTRADICCIÓN IMPORTANTE NO RESUELTA: el precio y las métricas de momentum aparecieron con dispersión muy alta entre fuentes en esta ronda (precio de $21,82 a $25,68 según fecha de caché; retorno a 1 año entre -13,37% y +44% según fuente) — se usó el par precio/marketCap más consistente y recientemente fechado, pero se dejaron en NULL `return6m`, `return1y`, `return3yCagr`, `distanceFromHigh52w`, `aboveSma200`, `peg` (0,08 vs 0,93, no reconciliable) y `maxDrawdown5y` por no encontrar una cifra fiable y coherente.
- Dejado en null también: `dividendCagr3y/5y/10y` (el recorte de 2022 hace que un CAGR no sea una medida de tendencia real), `revenueGrowthCagr3y/5y`, `epsGrowthCagr3y/5y`, `fcfGrowthCagr3y`, `operatingMarginTrend5y`, `earningsStability`, `interestCoverage`, `perVsHistoricalAvg5y`, `volatility3y`.

### Alphabet (GOOGL)
- DISTORSIÓN IMPORTANTE EN EL TRIMESTRE MÁS RECIENTE: en el 2T 2026 Alphabet registró una ganancia extraordinaria de ~$99.000M en "other income" por revalorización no realizada de inversiones en renta variable, que disparó el beneficio neto trimestral +298% (a $112.100M) y el EPS trimestral a $9,11 — sec.gov 8-K, finance.yahoo.com. Esto infla artificialmente el EPS TTM, el PER trailing, el ROE y el margen neto (aparecían en fuentes como 54,7%/93,6%, no representativos del negocio recurrente).
- Añadido: precio ($338,72) y marketCap ($4,14 billones), `dividendYield` (0,26%), `payoutRatioFCF` (20%), `consecutiveYearsPaying` (2, Alphabet solo empezó a pagar dividendo en 2024), `dividendCoverage` (4,95), `revenueGrowthCagr3y` (12,52%) y `revenueGrowthCagr5y` (17,15%) — CÁLCULO PROPIO a partir de cifras de ingresos anuales oficiales verificables (2020: $182.500M, 2022: $282.800M [10-K], 2025: $402.836M [comunicado 4T2025]), no un dato ya calculado de una fuente. `roic` (24,93%) y `operatingMargin` (33%) sí incluidos por basarse en resultado operativo, no afectado por la ganancia no operativa. `forwardPer` (16,5, cluster de fuentes 16,4-16,7; se descartó un outlier de 25,39 de stockanalysis.com), `peg` (1,93), `evEbitda` (23,20), `priceToFcf` (~76, cálculo propio, FCF deprimido por capex de IA ~$132.000M TTM), `priceToSales` (9,28, cálculo propio), `beta` (1,22), `distanceFromHigh52w` (20,7%, consistente con el precio: 52w high $408,61), `aboveSma200` (true, SMA200 $331,84). Fuentes: stockanalysis.com, 9to5google.com, finance.yahoo.com, sec.gov, gurufocus.com, valueinvesting.io, macrotrends.net. Consulta: 8-sep-2026.
- `per`, `roe`, `netMargin`, `epsGrowthCagr3y/5y`, `payoutRatioEarnings` dejados en NULL por la distorsión del EPS TTM ya explicada (denominador/numerador afectado por la ganancia no operativa del 2T).
- `netDebtToEbitda` dejado en null: Alphabet tiene posición de caja neta (caja $242.000M frente a deuda mucho menor), no aplica bien un ratio de apalancamiento (mismo criterio ya usado con Inditex en el universo IBEX35).
- Dejado en null también: `consecutiveYearsIncreasing` y `dividendCagr*` (historial de dividendo insuficiente, solo 2 años), `fcfGrowthCagr3y`, `operatingMarginTrend5y`, `earningsStability`, `interestCoverage`, `perVsHistoricalAvg5y`, `volatility3y`, `maxDrawdown5y`, `return6m`, `return1y`, `return3yCagr` (cifras de retorno a 1 año contradictorias entre fuentes, 46,7% vs 71,2%, no reconciliadas).

### Coca-Cola (KO)
- Añadido: precio ($88,81) y marketCap ($378.930M, consistente con 4.300M acciones), `dividendYield` (2,38%), `payoutRatioEarnings` (63,7%, coincide exactamente entre cálculo directo dividendo/EPS y cifra citada por fuentes), `payoutRatioFCF` (65,5%), `consecutiveYearsPaying` (106, desde 1920, dato de dominio público muy consolidado pero no verificado con búsqueda dedicada esta ronda), `consecutiveYearsIncreasing` (64, Dividend King — feb 2026, de $0,51 a $0,53/trim.), `dividendCoverage` (1,57), `roic` (20,10%), `roe` (42,05%), `operatingMargin` (31,85%), `netMargin` (28,56%, coincide exactamente con cálculo directo beneficio/ingresos), `netDebtToEbitda` (1,67, CÁLCULO PROPIO: EV implícito $406.820M [marketCap + deuda neta] / EBITDA implícito $16.710M [EV ÷ EV/EBITDA 24,34]), `per` (26,5), `forwardPer` (26,38), `peg` (3,56), `evEbitda` (24,34), `priceToFcf` (27,39), `priceToSales` (7,56, cálculo propio), `beta` (0,34), `distanceFromHigh52w` (4%, calculado del rango 52 semanas $65,35–$92,49). Fuentes: stockanalysis.com, gurufocus.com, finance.yahoo.com, fool.com, thestreet.com, tradingview.com. Consulta: 8-sep-2026.
- Pequeña discrepancia de ROIC entre fuentes (20,10% vs 18,54% según fecha) — se mantuvo la cifra más reciente.
- LIMITACIÓN: por agotamiento del cupo de búsquedas de la sesión, no se completó la investigación de `return6m`, `return1y`, `return3yCagr`, `aboveSma200` (se obtuvo el rango de 52 semanas pero no la SMA200 ni los retornos históricos), ni los `dividendCagr*` ni los CAGR de crecimiento (ingresos, BPA, FCF) ni volatilidad/máximo drawdown — quedan en null, se recomienda una ronda de seguimiento.

### Microsoft (MSFT)
- Añadido: precio ($499,70) y marketCap ($3,67 billones europeos) de stockanalysis.com, con dispersión menor entre fuentes (cierres entre $492,84 y $499,99 según fecha exacta, 3-8 sep 2026). `dividendYield` (0,73%), `payoutRatioEarnings` (21,9%), PER (27,84), forwardPER (25,30), PEG (1,57), EV/EBITDA (19,37), ROE (34,04%), ROIC (26,18%), beta (1,11) y rango 52 semanas ($349,20–$553,72) de stockanalysis.com/statistics. `operatingMargin` (45,11%) y `netMargin` (40,31%, coincide casi exactamente con el cálculo directo beneficio $133.750M/ingresos $331.840M TTM) de macrotrends.net. `priceToFcf` (54,8) y `priceToSales` (11,06) son CÁLCULO PROPIO (marketCap/FCF TTM $66.990M y marketCap/revenue TTM), coherente con el EV/FCF de 56,17 citado por stockanalysis (que usa enterprise value en vez de marketCap). `netDebtToEbitda` (0,42) cálculo propio (deuda neta $81.940M / EBITDA implícito ~$193.700M). `interestCoverage` (50,88x) de businessquant.com. `dividendCoverage` (2,51x) cálculo propio. `consecutiveYearsPaying`=23 (dividendo ininterrumpido desde 2003). `dividendCagr3y/5y/10y` (10,28%/9,91%/11,71%) de financecharts.com. `return1y` (-4,00%) y `return3yCagr` (17,59%) de financecharts.com. `distanceFromHigh52w` (9,8%) cálculo propio. `aboveSma200`=true. Fuentes: stockanalysis.com, macrotrends.net, gurufocus.com, financecharts.com, businessquant.com, mlq.ai, fool.com. Consulta: 8-sep-2026.
- AMBIGÜEDAD NO RESUELTA: `consecutiveYearsIncreasing` se deja en NULL — las fuentes discrepan de forma importante sobre la racha de subidas consecutivas del dividendo (cifras citadas entre 21 y 24 años según la fuente).
- Dejado en null también: `fcfGrowthCagr3y`, `operatingMarginTrend5y`, `earningsStability`, `perVsHistoricalAvg5y`, `return6m`, `volatility3y` (una fuente citó 38,99% pero con metodología/periodo poco claros) y `maxDrawdown5y` (candidato histórico conocido: -37,6% nov-2021/nov-2022; pero en 2026 hubo otra fuerte corrección por el capex de IA, con el rango de 52 semanas implicando una caída similar dentro del último año, sin cifra verificada a 5 años que reconcilie ambos episodios).

### Warner Bros Discovery (WBD)
- IMPORTANTE — SITUACIÓN CORPORATIVA EXCEPCIONAL: la escisión en dos compañías (Warner Bros. / Discovery Global) anunciada en 2025 quedó SUSTITUIDA por una operación distinta — Paramount Skydance acordó ADQUIRIR el 100% de WBD en efectivo a $31,00/acción (oferta ganadora tras una puja con Netflix, aprobada por accionistas en abril 2026, con autorización de DOJ/UE de junio 2026, aunque un grupo de fiscales generales estatales liderados por California ha presentado una demanda antimonopolio). El cierre, previsto originalmente para el 30-sep-2026, se ha retrasado hasta como muy tarde junio de 2027. A fecha de esta ficha WBD sigue cotizando como entidad única bajo el ticker WBD en NASDAQ (la escisión en dos compañías nunca se completó), pero es un valor en situación de arbitraje de fusión: su precio ($28,27, ~9% por debajo de la oferta de $31) y su revalorización a 1 año (+137,6%) reflejan sobre todo la puja Paramount/Netflix y no la evolución operativa del negocio.
- Añadido: precio y marketCap ($71.130M, 2,51bn acciones) de heygotrade.com/stockanalysis.com (3-8 sep 2026). `netDebtToEbitda` (3,4x) es EL DATO REPORTADO DIRECTAMENTE POR LA PROPIA COMPAÑÍA (nota de prensa de resultados 2T 2026 en wbd.com: deuda neta ~$30.000M, "net leverage 3.4x") — la fuente más fiable posible. `roic` (3,36%), `roe` (-4,96%), `evEbitda` (12,78), `priceToSales` (1,3) y `beta` (1,57) de stockanalysis.com. `netMargin` (-4,7%) cálculo propio (pérdida neta TTM -$1.740M / ingresos TTM $37.210M) — el resultado neto TTM está distorsionado por una comisión de terminación de $2.800M pagada a Netflix en el 1T 2026 tras perder la puja inicial. `priceToFcf` (30,8) cálculo propio (marketCap/FCF TTM $2.310M). Fuentes: stockanalysis.com, wbd.com (notas de prensa oficiales de resultados 1T/2T 2026), heygotrade.com, macrotrends.net, sec.gov (10-K FY2025, 8-K resultados), cnbc.com, britannica.com/money, financecharts.com. Consulta: 8-sep-2026.
- SIN DIVIDENDO: WBD no paga dividendo (10-K propio, "no present intention to do so") — `dividendYield`/`payoutRatioEarnings`/`payoutRatioFCF`/`consecutiveYearsPaying` en 0 por ser un hecho verificado; el resto de campos de dividendo en NULL por no aplicar.
- Dejado en NULL por baja fiabilidad/incoherencia entre fuentes: `per`/`forwardPer` (beneficio negativo, y una fuente cita un forwardPER de 1383,73 sin sentido económico), `peg`, `interestCoverage` (resultado operativo TTM distorsionado por la comisión a Netflix y deterioros); `revenueGrowthCagr3y/5y`, `epsGrowthCagr3y/5y`, `fcfGrowthCagr3y`, `operatingMargin`, `operatingMarginTrend5y`, `earningsStability` (la entidad WBD nace de la fusión WarnerMedia-Discovery de abril 2022, los CAGR a 3-5 años mezclan periodos no comparables, igual criterio que con AT&T); `volatility3y`/`maxDrawdown5y` (historial de precio 2022-2026 combina el desplome post-fusión con la revalorización por la puja de adquisición, sin cifra a 5 años reconciliada); `perVsHistoricalAvg5y`; `return6m`/`return3yCagr`; `aboveSma200` (fuentes contradictorias sobre la SMA200).

### Limitación general de esta ronda
Igual que en las rondas del IBEX35, sin acceso a fetch directo de páginas (EGRESS_BLOCKED también confirmado vía `curl` directo, no solo WebFetch). Todos los datos proceden de resúmenes/snippets de búsqueda. Varios ratios de valoración y de crecimiento (netDebtToEbitda de JNJ/KO/MSFT, priceToFcf de GIS/T/GOOGL/MSFT/WBD, priceToSales de GOOGL/KO/MSFT, revenueGrowthCagr3y/5y de GOOGL) se calcularon manualmente a partir de componentes oficiales verificados cuando las cifras ya calculadas por fuentes externas eran contradictorias entre sí o no se encontraron — el razonamiento y las cifras base de cada cálculo están documentados en el `sourceNote` de cada empresa. La limitación más importante de la primera pasada de esta ronda fue el agotamiento del cupo de búsquedas web de la sesión, que impidió completar MSFT y WBD en ese momento (completadas en la ronda de seguimiento) y dejó varias métricas de KO sin investigar (ver notas de cada empresa).
