...
Investigación de 7 empresas estadounidenses (S&P 500) recién añadidas a `lib/data/providers/globalCompanies.ts` (fichero de ampliación del universo, distinto de `ibex35Companies.ts`), posiciones reales de la cartera personal del propietario del sitio: JNJ, GIS, T, GOOGL, KO, MSFT, WBD. Recopilación por búsqueda web (Claude WebSearch) el 8-sep-2026. Igual que en las rondas del IBEX35, el fetch directo de páginas (stockanalysis.com, sec.gov, wikipedia.org, etc.) devolvió `EGRESS_BLOCKED` en este entorno, tanto vía WebFetch como vía `curl` directo (confirmado con `curl -sS .../__agentproxy/status`, que muestra el dominio rechazado por política de la organización) — todos los datos proceden de resúmenes/snippets de WebSearch que citan esas fuentes, contrastados entre sí cuando ha sido posible. **LIMITACIÓN IMPORTANTE DE ESTA RONDA: el cupo de búsquedas web de la sesión (compartido con otros agentes trabajando en paralelo sobre el resto del universo, 200 llamadas) se agotó tras completar 5 de las 7 empresas** — MSFT y WBD quedaron sin investigar y sus fichas mantienen el placeholder (`price: 0`, resto en `null`) con una nota explicando el motivo. Se recomienda una ronda de seguimiento específica para estas dos.
...
### Microsoft (MSFT) y Warner Bros Discovery (WBD) — SIN INVESTIGAR
El cupo de búsquedas web de la sesión (200 llamadas, compartido con otros agentes trabajando en paralelo en el resto del universo) se agotó justo después de completar Coca-Cola, antes de poder investigar estas dos empresas. Se intentó además acceso directo vía WebFetch y `curl` (confirmado con el estado del proxy de red del entorno), pero todos los dominios financieros y de referencia (incluida wikipedia.org y sec.gov) están bloqueados por política de red (`EGRESS_BLOCKED`) — WebSearch era la única vía disponible y no quedaba cupo. Siguiendo la regla de no inventar cifras, sus fichas mantienen el placeholder original (`price: 0`, resto en `null`) con una nota explicando el motivo, en vez de estimar valores de memoria (que además estarían desactualizados frente a septiembre de 2026). Se recomienda una ronda de seguimiento dedicada a estas dos empresas con cupo de búsquedas disponible. Nota adicional para WBD: la compañía está en pleno proceso de escisión en dos entidades (anunciado en 2025), lo que conviene verificar antes de investigarla.

### Limitación general de esta ronda
Igual que en las rondas del IBEX35, sin acceso a fetch directo de páginas (EGRESS_BLOCKED también confirmado vía `curl` directo, no solo WebFetch). Todos los datos proceden de resúmenes/snippets de búsqueda. Varios ratios de valoración y de crecimiento (netDebtToEbitda de JNJ/KO, priceToFcf de GIS/T/GOOGL, priceToSales de GOOGL/KO, revenueGrowthCagr3y/5y de GOOGL) se calcularon manualmente a partir de componentes oficiales verificados cuando las cifras ya calculadas por fuentes externas eran contradictorias entre sí o no se encontraron — el razonamiento y las cifras base de cada cálculo están documentados en el `sourceNote` de cada empresa. La limitación más importante de esta ronda fue el agotamiento del cupo de búsquedas web de la sesión, que impidió completar MSFT y WBD y dejó varias métricas de KO sin investigar (ver notas de cada empresa).
